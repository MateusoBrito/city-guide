...
export default function Navbar() {
  ...
  return (
    <nav className="h-16" style={{ backgroundColor: "var(--secondary)" }}>
      <div className="w-full mx-auto h-full px-6 flex items-center justify-between">
        ...
        <div className="flex items-center gap-4">
          ...
          {status !== "authenticated" ? (
            ...
            /* Adicionada a referência 'dropdownRef' e removido o instável 'onBlur' */
            <div className="relative z-10" ref={dropdownRef}>
              ...
              >
                <div className="w-9 h-9 rounded-full bg-white/20 flex items-center justify-center text-white group-hover:bg-white/30 transition overflow-hidden">
                  {session?.user?.image ? (
                    <img 
                      src={session.user.image} 
                      alt="Avatar" 
                      referrerPolicy="no-referrer" /* 👈 IMPORTANTE: Evita bloqueios de imagem pelo Google Auth */
              ...
              {dropdownAberto && (
                <div className="absolute right-0 mt-2 w-48 bg-white border border-slate-200 rounded-xl shadow-lg py-2 z-50">
                  ...
                  > 
                    <Users size={16} className="text-slate-400" /> Meu Perfil 
                  </button>
